<x-layouts.app :title="__('app.mock_tests').' · '.__('app.writing')">
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            ...
            <h1 class="mt-4 text-3xl font-semibold">{{ __('app.writing') }} Mock</h1>
        ...
        @forelse($tasks as $task)
            <div class="rounded-2xl border border-slate-200 bg-white/95 p-5 shadow-sm">
                <div class="flex items-start justify-between gap-2">
                    ...
                    <span class="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-semibold uppercase text-slate-600">MOCK</span>
                </div>
                <p class="mt-3 text-sm text-slate-500">{{ \Illuminate\Support\Str::limit($task->prompt, 130) }}</p>
                <div class="mt-4 flex flex-wrap items-center gap-3 text-xs text-slate-500">
                    <span class="rounded-full bg-slate-100 px-2 py-1 uppercase">{{ strtoupper((string) $task->task_type) }}</span>
                ...
                @if($canAccess)
                    <a href="{{ route('mock.writing.show', $task) }}" class="mt-4 inline-flex rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white">
                        {{ __('app.start_mock') }}
                    </a>
                @else
                    <button type="button"
        ...
        <div class="mt-10 rounded-2xl border border-slate-200 bg-white/95 p-6 shadow-sm">
            ...
            <div class="mt-4 space-y-3">
                @foreach($recentSubmissions as $submission)
                    <a href="{{ route('mock.writing.result', $submission) }}" class="flex items-center justify-between rounded-xl border border-slate-200 px-4 py-3 text-sm hover:bg-slate-50">
