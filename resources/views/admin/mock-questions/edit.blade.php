<x-layouts.app :title="__('app.edit_question')">
    <div class="mx-auto max-w-4xl space-y-6">
        <div>
            ...
            <h1 class="mt-4 text-3xl font-semibold">{{ __('app.edit_question') }}</h1>
            <p class="mt-2 text-sm text-slate-500">{{ $test->title }} · {{ __('app.section_number') }} {{ $section->section_number }}</p>
        </div>

        <form method="POST" action="{{ route('admin.mock-test-sections.questions.update', [$test, $section, $question]) }}" class="space-y-4 rounded-2xl border border-slate-200 bg-white/95 p-6 shadow-sm">
            @csrf
            @method('PUT')
            @include('admin.mock-questions.partials.form', ['question' => $question])
