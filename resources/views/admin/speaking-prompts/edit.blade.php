<x-layouts.app :title="__('app.edit_speaking_prompt')">
    <div class="flex items-center justify-between">
        <div>
            ...
            <h1 class="mt-4 text-3xl font-semibold">{{ __('app.edit_speaking_prompt') }}</h1>
        </div>
        <a href="{{ route('admin.speaking-prompts.index') }}" class="text-sm text-slate-500 hover:text-slate-700">{{ __('app.back_to_list') }}</a>
    </div>

    <form method="POST" action="{{ route('admin.speaking-prompts.update', $prompt) }}" class="mt-6 rounded-2xl border border-slate-200 bg-white/95 p-6 shadow-sm">
        @csrf
        @method('PUT')
        @include('admin.speaking-prompts._form', ['prompt' => $prompt])
