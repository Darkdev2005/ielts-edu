<x-layouts.app :title="__('app.add_rule')">
    <div class="flex items-center justify-between">
        <div>
            ...
            <h1 class="mt-4 text-3xl font-semibold">{{ __('app.add_rule') }}</h1>
    ...
    <form method="POST" action="{{ route('admin.grammar.rules.store', $topic) }}" enctype="multipart/form-data" class="mt-8 space-y-4 rounded-2xl border border-slate-200 bg-white/95 p-6 shadow-sm">
        @csrf
        <div class="grid gap-4 md:grid-cols-3">
            <div>
                <label class="block text-sm font-medium">{{ __('app.rule_key') }}</label>
                <input name="rule_key" value="{{ old('rule_key') }}" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2" />
            ...
            <div>
                ...
                <select name="rule_type" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2">
                    @foreach(['core','usage','note','exception'] as $type)
                        <option value="{{ $type }}" @selected(old('rule_type', 'core') === $type)>{{ strtoupper($type) }}</option>
                    @endforeach
                </select>
                @error('rule_type')
                    <div class="mt-1 text-xs text-rose-600">{{ $message }}</div>
            ...
            <div>
                ...
                <select name="cefr_level" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2">
                    ...
                    @foreach(['A1','A2','B1','B2','C1'] as $level)
                        <option value="{{ $level }}" @selected(old('cefr_level') === $level)>{{ $level }}</option>
                    @endforeach
                </select>
                @error('cefr_level')
                    <div class="mt-1 text-xs text-rose-600">{{ $message }}</div>
        ...
        <div class="grid gap-4 md:grid-cols-3">
            <div>
                ...
                <textarea name="rule_text_uz" rows="4" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2">{{ old('rule_text_uz') }}</textarea>
                @error('rule_text_uz')
                    <div class="mt-1 text-xs text-rose-600">{{ $message }}</div>
                @enderror
            </div>
            <div>
                ...
                <textarea name="rule_text_en" rows="4" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2">{{ old('rule_text_en') }}</textarea>
                @error('rule_text_en')
                    <div class="mt-1 text-xs text-rose-600">{{ $message }}</div>
                @enderror
            </div>
            <div>
                ...
                <textarea name="rule_text_ru" rows="4" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2">{{ old('rule_text_ru') }}</textarea>
                @error('rule_text_ru')
                    <div class="mt-1 text-xs text-rose-600">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div>
            ...
            <textarea name="formula" rows="2" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2">{{ old('formula') }}</textarea>
            @error('formula')
                <div class="mt-1 text-xs text-rose-600">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="grid gap-4 md:grid-cols-3">
            <div>
                ...
                <textarea name="example_uz" rows="3" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2">{{ old('example_uz') }}</textarea>
            </div>
            <div>
                <label class="block text-sm font-medium">{{ __('app.example_en') }}</label>
                <textarea name="example_en" rows="3" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2">{{ old('example_en') }}</textarea>
            </div>
            <div>
                <label class="block text-sm font-medium">{{ __('app.example_ru') }}</label>
                <textarea name="example_ru" rows="3" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2">{{ old('example_ru') }}</textarea>
            </div>
        </div>
        ...
        <div class="grid gap-4 md:grid-cols-2">
            <div>
                ...
                <textarea name="negative_example" rows="2" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2">{{ old('negative_example') }}</textarea>
            </div>
            <div>
                <label class="block text-sm font-medium">{{ __('app.common_mistake') }}</label>
                <textarea name="common_mistake" rows="2" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2">{{ old('common_mistake') }}</textarea>
            </div>
        </div>
        <div>
            <label class="block text-sm font-medium">{{ __('app.correct_form') }}</label>
            <textarea name="correct_form" rows="2" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2">{{ old('correct_form') }}</textarea>
        </div>
        ...
        <div>
            ...
            <input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2" />
            @error('sort_order')
                <div class="mt-1 text-xs text-rose-600">{{ $message }}</div>
            @enderror
        </div>
