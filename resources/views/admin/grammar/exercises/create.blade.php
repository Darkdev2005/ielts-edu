<x-layouts.app :title="__('app.add_exercise')">
    <div class="flex items-center justify-between">
        <div>
            ...
            <h1 class="mt-4 text-3xl font-semibold">{{ __('app.add_exercise') }}</h1>
    ...
    <form method="POST" action="{{ route('admin.grammar.exercises.store', $topic) }}" class="mt-8 space-y-4 rounded-2xl border border-slate-200 bg-white/95 p-6 shadow-sm">
        @csrf
        <div>
            ...
            <select name="grammar_rule_id" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2">
                @foreach($rules as $rule)
                    @if($rule->rule_key !== '__unmapped__')
                        <option value="{{ $rule->id }}" @selected(old('grammar_rule_id') == $rule->id)>
        ...
        <div>
            ...
            <select name="exercise_type" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2">
                @foreach(['mcq','gap','tf','reorder'] as $type)
                    <option value="{{ $type }}" @selected(old('exercise_type', 'mcq') === $type)>{{ strtoupper($type) }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <textarea name="question" rows="3" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2">{{ old('question') }}</textarea>
            @error('question')
                <div class="mt-1 text-xs text-rose-600">{{ $message }}</div>
            @enderror
        </div>
        <div class="grid gap-3 md:grid-cols-2">
            <div>
                ...
                <input name="option_a" value="{{ old('option_a') }}" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2" />
            </div>
            <div>
                <label class="block text-sm font-medium">{{ __('app.option_b') }}</label>
                <input name="option_b" value="{{ old('option_b') }}" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2" />
            </div>
            <div>
                <label class="block text-sm font-medium">{{ __('app.option_c') }}</label>
                <input name="option_c" value="{{ old('option_c') }}" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2" />
            </div>
            <div>
                <label class="block text-sm font-medium">{{ __('app.option_d') }}</label>
                <input name="option_d" value="{{ old('option_d') }}" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2" />
            </div>
        </div>
        @error('options')
        ...
        <div>
            ...
            <input name="correct_answer" value="{{ old('correct_answer', 'A') }}" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2" />
        ...
        <div class="grid gap-4 md:grid-cols-3">
            <div>
                ...
                <textarea name="explanation_uz" rows="3" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2">{{ old('explanation_uz') }}</textarea>
            </div>
            <div>
                <label class="block text-sm font-medium">{{ __('app.explanation_en') }}</label>
                <textarea name="explanation_en" rows="3" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2">{{ old('explanation_en') }}</textarea>
            </div>
            <div>
                <label class="block text-sm font-medium">{{ __('app.explanation_ru') }}</label>
                <textarea name="explanation_ru" rows="3" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2">{{ old('explanation_ru') }}</textarea>
            </div>
        </div>
