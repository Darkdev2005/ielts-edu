<x-layouts.app :title="__('app.new_topic')">
    <div class="flex items-center justify-between">
        <div>
            ...
            <h1 class="mt-4 text-3xl font-semibold">{{ __('app.new_topic') }}</h1>
        </div>
        <a href="{{ route('admin.grammar.topics.index') }}" class="text-sm text-slate-500 hover:text-slate-700">{{ __('app.back_to_list') }}</a>
    </div>

    <form method="POST" action="{{ route('admin.grammar.topics.store') }}" class="mt-8 space-y-4 rounded-2xl border border-slate-200 bg-white/95 p-6 shadow-sm">
        @csrf
        <div>
            <label class="block text-sm font-medium">{{ __('app.topic_key') }}</label>
            <input name="topic_key" value="{{ old('topic_key') }}" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2" />
        ...
        <div class="grid gap-4 md:grid-cols-3">
            <div>
                ...
                <input name="title_uz" value="{{ old('title_uz') }}" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2" />
                @error('title_uz')
                    <div class="mt-1 text-xs text-rose-600">{{ $message }}</div>
                @enderror
            </div>
            <div>
                ...
                <input name="title_en" value="{{ old('title_en') }}" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2" />
                @error('title_en')
                    <div class="mt-1 text-xs text-rose-600">{{ $message }}</div>
                @enderror
            </div>
            <div>
                ...
                <input name="title_ru" value="{{ old('title_ru') }}" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2" />
                @error('title_ru')
                    <div class="mt-1 text-xs text-rose-600">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="grid gap-4 md:grid-cols-3">
            <div>
                ...
                <textarea name="description_uz" rows="3" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2">{{ old('description_uz') }}</textarea>
            ...
            <div>
                ...
                <textarea name="description_en" rows="3" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2">{{ old('description_en') }}</textarea>
            ...
            <div>
                ...
                <textarea name="description_ru" rows="3" class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2">{{ old('description_ru') }}</textarea>
